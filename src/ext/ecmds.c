...
int run() {
    ...
    // If an output file is specified, use this as a counter.
    if (*outf != '\0') tempouts_statuses[index] = USE_AS_COUNTER;
    else {
        openpipe();
        if (piping == 1 || piping == 3)
            tempouts_statuses[index] = USE_AS_COUNTER;
        else tempouts_statuses[index] = BUSY;
    }
    pid_t childpid = fork();
    if (childpid == 0) {  // child
        closechildend();
...
int main(int argc, char** argv) {
    ...
    while ( (line_length=getline(&buff, &n, input)) != -1) {
        unparsed = buff;  // set the unparsed string as the whole line
        while (*unparsed != '\0') {
            tokenallocation();
            splittokens();
            noa = parse();
